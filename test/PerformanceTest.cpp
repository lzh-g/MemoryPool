...
public:
    ...
    static void testSmallAllocation()
    {
        ...
        std::cout << "\nTesting small allocations (" << NUM_ALLOCS << " allocations of " << SMALL_SIZE << " bytes): \n";
    ...
    static void testMultiThreaded()
    {
        ...
        // 测试new/delete
        {
            ...
            for (size_t i = 0; i < NUM_THREADS; ++i)
            {
                // emplace_back可以直接传递对象的参数，使得直接在容器中构造对象，避免了push_back的拷贝和移动开销
    ...
    static void testMixedSizes()
    {
        ...
        std::cout << "\nTesting mixed size allocations (" << NUM_ALLOCS << " allocations): " << std::endl;
...
int main()
{
    std::cout << "Starting perforance tests..." << std::endl;
